...
func shouldSkipPassthroughHeader(name string, connectionHeaderValues ...string) bool {
	return common.ShouldSkipClientHeaderForUpstreamWithConnectionTokens(name, connectionHeaderValues...)
}

func incomingConnectionHeaderValues(c *gin.Context) []string {
	if c == nil || c.Request == nil {
		return nil
	}
	return c.Request.Header.Values("Connection")
}

func applyHeaderOverridePlaceholders(targetHeader string, template string, c *gin.Context, apiKey string, connectionHeaderValues []string) (string, bool, error) {
	...
	if strings.HasPrefix(trimmed, clientHeaderPlaceholderPrefix) {
		...
		if shouldSkipPassthroughHeader(name, connectionHeaderValues...) ||
			shouldSkipPassthroughHeader(targetHeader, connectionHeaderValues...) {
			return "", false, nil
		}
		if c == nil || c.Request == nil {
...
func processHeaderOverride(info *common.RelayInfo, c *gin.Context) (map[string]string, error) {
	...
	connectionHeaderValues := incomingConnectionHeaderValues(c)
		...
		for name := range c.Request.Header {
			...
			if !passAll {
				...
				for _, re := range passthroughRegex {
					if re.MatchString(normalizedName) {
						matched = true
						break
					}
				}
			...
			headerOverride[normalizedName] = value
		}
	}
	...
	for k, v := range headerOverrideSource {
		...
		value, include, err := applyHeaderOverridePlaceholders(key, str, c, info.ApiKey, connectionHeaderValues)
		if err != nil {
			return nil, types.NewError(err, types.ErrorCodeChannelHeaderOverrideInvalid)
		}
		if !include {
